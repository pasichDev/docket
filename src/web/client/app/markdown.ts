...
export const DESC_PREVIEW_CHARS = 300;
...
function mdInline(escaped: string): string {
  ...
  const link = (href: string, label: string): string => hold(`<a href="${href}" target="_blank" rel="noopener noreferrer">${label}</a>`);
...
export function renderMarkdown(src: string | null | undefined): string {
  ...
  const openList = (kind: "ul" | "ol"): void => { if (list !== kind) { closeList(); out.push("<" + kind + ">"); list = kind; } };
...
export function clampMarkdown(src: string | null | undefined, max: number): { text: string; truncated: boolean } {
